...
class _ChartsScreenState extends State<ChartsScreen> {
  ...
  List<FlSpot> _generateSpots() {
    ...
    for (int i = 0; i < records.length; i++) {
      final value = SettingsService()
          .convertToDisplayUnit(records[i].bloodSugar, _displayUnit);
  ...
  Widget build(BuildContext context) {
    ...
    final avg = SettingsService()
        .convertToDisplayUnit(_calculateAverage(), _displayUnit);
    final min =
        SettingsService().convertToDisplayUnit(_calculateMin(), _displayUnit);
    final max =
        SettingsService().convertToDisplayUnit(_calculateMax(), _displayUnit);
                        ...
                        children: [
                          Expanded(
                            child: _buildStatCard(
                                'Average', '${avg.toStringAsFixed(1)} $unit'),
                          ),
                          const SizedBox(width: 16),
                          Expanded(
                            child: _buildStatCard('Range',
